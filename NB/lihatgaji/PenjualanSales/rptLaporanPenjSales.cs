namespace NB
{
    ...
    /// </summary>
    public partial class rptLaporanPenjSales : Telerik.Reporting.Report
    {
        private string value_ = "0";
        private string value2_ = "0";
        private string value3_ = "0";
        public rptLaporanPenjSales()
        {
            //
        ...
        public static string GetDaerah(string SalesShort,DateTime tanggal)
        {
            string daerah =string.Empty;
            DataTable dt = new DataTable("Test");
            //StringBuilder sQuery = new StringBuilder();
            //sQuery.Append("call pgetdatapenjualandaerahsales('" + tanggal.ToString("yyyy-MM-dd") 
            //              + "' ,'" +SalesShort  + "')");
            ...
            adapt.SelectCommand.CommandText = "pgetdatapenjualandaerahsales";
            adapt.SelectCommand.Parameters.AddWithValue("salesshort_", SalesShort);
            adapt.SelectCommand.Parameters.AddWithValue("tanggal_", tanggal);            
            adapt.Fill(dt);
            if (dt.Rows.Count > 0)
            {
        ...
        public static string GetBotolPrices(string SalesShort,DateTime tanggal)
        {
            StringBuilder daerah = new StringBuilder();
            DataTable dt = new DataTable("Test");
            //StringBuilder sQuery = new StringBuilder();
            //sQuery.Append("pgetdatapenjualanpackagesales('" + SalesShort 
            //    + "','" + tanggal.ToString("yyyy-MM-dd") + "')"); 
            MySqlDataAdapter adapt = new MySqlDataAdapter(""
                                    , ConfigurationManager.ConnectionStrings["NB.Properties.Settings.sinarekConnectionString"].ConnectionString);
            adapt.SelectCommand.CommandType = CommandType.StoredProcedure;
            adapt.SelectCommand.CommandText = "pgetdatapenjualanpackagesales";
            adapt.SelectCommand.Parameters.AddWithValue("salesshort_", SalesShort);
            adapt.SelectCommand.Parameters.AddWithValue("tanggal_", tanggal); 
            adapt.Fill(dt);
            if (dt.Rows.Count > 0)
        ...
        private void rptLaporanPenj_ItemDataBound(object sender, EventArgs e)
        {
            ...
            textBox7.Value = "=" + (decimal.Parse(value3_) - decimal.Parse(value_)).ToString();
            textBox1.Value = "=" + (decimal.Parse(value2_));
            textBox10.Value = "=" + (decimal.Parse(value3_) - decimal.Parse(value_) + decimal.Parse(value2_)).ToString();
        }
